...
import { renderNotificationText, RATE_LIMIT_UNAVAILABLE_LINE } from "./render.js";
...
describe("renderNotificationText", () => {
  it("renders completion without usage — shows unavailable fallback", () => {
  ...
  it("approval-wait without usage or approval — header + unavailable fallback", () => {
    const ev = makeEvent("approval-wait");
    const text = renderNotificationText(ev, BASE_CONFIG);
    const lines = text.split("\n");
    expect(lines).toHaveLength(2); // header + unavailable
    expect(lines[0]).toBe("⏸️ Codex waiting for approval");
    expect(lines[1]).toBe(RATE_LIMIT_UNAVAILABLE_LINE);
  });
  ...
  it("renders unavailable fallback when usage has empty buckets array", () => {
    const ev = makeEvent("completion", {
      usage: { displayMode: "left", buckets: [] },
    });
    const text = renderNotificationText(ev, BASE_CONFIG);
    expect(text).toBe(`✅ Codex complete\n${RATE_LIMIT_UNAVAILABLE_LINE}`);
  });

  it("approval-wait with description but no usage — description + unavailable fallback", () => {
    const ev = makeEvent("approval-wait", {
      approval: { descriptionLine: "Write file src/index.ts" },
    });
    const text = renderNotificationText(ev, BASE_CONFIG);
    const lines = text.split("\n");
    expect(lines).toHaveLength(3); // header + description + unavailable
    expect(lines[0]).toBe("⏸️ Codex waiting for approval");
    expect(lines[1]).toBe("Write file src/index.ts");
    expect(lines[2]).toBe(RATE_LIMIT_UNAVAILABLE_LINE);
  });

  it("rate-limit-reached without usage — no unavailable fallback (event always has usage)", () => {
    const ev = makeEvent("rate-limit-reached");
    const text = renderNotificationText(ev, BASE_CONFIG);
    expect(text).toBe("⛔ Codex rate limit reached");
    expect(text).not.toContain(RATE_LIMIT_UNAVAILABLE_LINE);
  });

  it("completion with usage — bars present, no unavailable fallback", () => {
    const ev = makeEvent("completion", {
      usage: {
        displayMode: "left",
        buckets: [
          { windowLabel: "5h", usedPercent: 27, resetsAt: null },
  ...
  it("approval-wait with usage — bars present, no unavailable fallback", () => {
    const ev = makeEvent("approval-wait", {
      approval: { descriptionLine: "Bash command requires approval." },
      usage: {
        displayMode: "left",
        buckets: [
          { windowLabel: "5h", usedPercent: 50, resetsAt: null },
          { windowLabel: "W", usedPercent: 20, resetsAt: null },
        ],
      },
    });
    const text = renderNotificationText(ev, BASE_CONFIG);
    const lines = text.split("\n");
    expect(lines[0]).toBe("⏸️ Codex waiting for approval");
    expect(lines[1]).toBe("Bash command requires approval.");
    expect(text).toContain("█");
    expect(text).not.toContain(RATE_LIMIT_UNAVAILABLE_LINE);
  });
});
