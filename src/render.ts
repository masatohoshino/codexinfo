...
export const RATE_LIMIT_UNAVAILABLE_LINE = "rate-limit: unavailable";
...
export function renderNotificationText(
  ...
  if (event.usage && event.usage.buckets.length > 0) {
    ...
    lines.push(...barLines);
  } else if (event.eventType === "completion" || event.eventType === "approval-wait") {
    lines.push(RATE_LIMIT_UNAVAILABLE_LINE);
  }
